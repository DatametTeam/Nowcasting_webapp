...
if is_hpc():
    ...
    # Use mock system for local development
...
def read_groundtruth_and_target_data(selected_key, selected_model):
    ...
    src_fold = Path(__file__).resolve().parent
...
def load_prediction_data(st, time_options, latest_file):
    ...
    if st.session_state.selected_model and st.session_state.selected_time:
        ...
        with h5py.File(os.path.join(src_dir, "resources/mask/radar_mask.hdf"), "r") as f:
            radar_mask = f["mask"][()]
...
src_dir = Path(__file__).resolve().parent
italy_shape = gpd.read_file(os.path.join(src_dir, "resources/shapefiles/italian_regions/gadm41_ITA_1.shp"))
